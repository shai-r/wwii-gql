...
from app.repository.mission_repository import find_mission_by_id, find_missions_by_date_range, \
    find_missions_by_targets
from app.repository.target_repository import find_targets_by_cities, find_targets_by_target_industry, \
...
class Query(ObjectType):
    ...
    def resolve_missions_by_country_id(root, info, country_id):
        return find_missions_by_targets(
            find_targets_by_cities(
                find_cities_by_country_id(country_id)
            ))
    ...
    def resolve_mission_by_target_industry(root, info, target_industry):
        return find_missions_by_targets(
            find_targets_by_target_industry(target_industry)
        )

    @staticmethod
    def resolve_mission_by_target_type_id(root, info, target_type_id):
        return find_missions_by_targets(
            find_targets_by_target_type_id(target_type_id)
        )
